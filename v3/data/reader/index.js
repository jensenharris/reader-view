...
const render = async () => {
  ...
  // Replace YouTube embeds with clickable thumbnails (embeds fail from chrome-extension:// origin)
  for (const yt of [...iframe.contentDocument.querySelectorAll('iframe[src*="youtube.com/embed/"]')]) {
    const m = yt.src.match(/youtube\.com\/embed\/([^?&#]+)/);
    if (!m) continue;
    const vid = m[1];
    ...
    link.style.cssText = 'display:block;position:relative;max-width:100%;aspect-ratio:16/9;border-radius:8px;overflow:hidden;background:#000;';
    const img = iframe.contentDocument.createElement('img');
    img.src = 'https://img.youtube.com/vi/' + vid + '/hqdefault.jpg';
    img.style.cssText = 'display:block;width:100%;height:100%;object-fit:cover;';
    const play = iframe.contentDocument.createElement('div');
    play.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;';
    play.innerHTML = '<svg width="68" height="48" viewBox="0 0 68 48"><path d="M66.52 7.74c-.78-2.93-2.49-5.41-5.42-6.19C55.79.13 34 0 34 0S12.21.13 6.9 1.55c-2.93.78-4.64 3.26-5.42 6.19C.06 13.05 0 24 0 24s.06 10.95 1.48 16.26c.78 2.93 2.49 5.41 5.42 6.19C12.21 47.87 34 48 34 48s21.79-.13 27.1-1.55c2.93-.78 4.64-3.26 5.42-6.19C67.94 34.95 68 24 68 24s-.06-10.95-1.48-16.26z" fill="red"/><path d="M45 24L27 14v20" fill="#fff"/></svg>';
